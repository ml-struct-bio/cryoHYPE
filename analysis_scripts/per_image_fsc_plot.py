...
def parse_args():
    ...
    parser.add_argument("--Apix", type=float, default=4.5, help="pixel size")
    return parser
...
def main(args):
    ...
    if args.mask == "True":
        if args.flip == "False":
            fsc_files = glob.glob(os.path.join(args.input, "fsc", file_pattern))
        else:
            fsc_files = glob.glob(os.path.join(args.input, "fsc_flipped", file_pattern))
    else:
        if args.flip == "False":
            fsc_files = glob.glob(os.path.join(args.input, "fsc_no_mask", file_pattern))
        else:
            fsc_files = glob.glob(os.path.join(args.input, "fsc_flipped_no_mask", file_pattern))
    ...
    if args.mask == "True":
        if args.flip == "False":
            plt.savefig(os.path.join(args.input, "fsc_auc.png"), dpi=1200, bbox_inches='tight')
        else:
            plt.savefig(os.path.join(args.input, "fsc_auc_flipped.png"), dpi=1200, bbox_inches='tight')
    else:
        if args.flip == "False":
            plt.savefig(os.path.join(args.input, "fsc_auc_no_mask.png"), dpi=1200, bbox_inches='tight')
        else:
            plt.savefig(os.path.join(args.input, "fsc_auc_flipped_no_mask.png"), dpi=1200, bbox_inches='tight')
    print('plot saved!')
