...
class ImageSource:
    ...
    def from_file(
        ...
        elif ext == "txt" and multi is None:
        ...
        elif ext == "txt" and multi:
            return TxtFileSourceMulti(
                filepath,
                lazy=lazy,
                indices=indices,
                max_threads=max_threads,
...
class TxtFileSourceMulti(ArraySource):
    def __init__(
        self,
        filepath: str,
        lazy: bool = True,
        ...
        _source_lengths = [MRCFileSource(path, lazy=False)._images(np.arange(multi)) for path in _paths]
        data = np.stack(_source_lengths)
