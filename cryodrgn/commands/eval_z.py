...
def add_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=int,
        default=40,
        help="Batch size for zs inference",
    )
    parser.add_argument(
        "--load-poses",
        action="store_true",
...
def main(args):
    ...
    with torch.no_grad():
        ...
        for i, batch in tqdm(enumerate(data_generator), total=len(data_generator)):
            ...
            z = model.eval_z(batch)
            zs.append(z)
            if (i + 1) % 25 == 0:
                zs = torch.cat(zs, dim=0)
                out_z = '{}/zs-{:03d}.pt'.format(args.outdir, counter)
                torch.save(zs, out_z)
                zs = []
                counter += 1
        # save one last time if the total number of images isn't a multipe of 25
        if len(zs) > 0:
            zs = torch.cat(zs, dim=0)
            out_z = '{}/zs-{:03d}.pt'.format(args.outdir, counter)
            torch.save(zs, out_z)