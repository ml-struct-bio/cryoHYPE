...
class Lattice(nn.Module):
    def __init__(
        self, D: int, extent: float = 0.5, ignore_DC: bool = True
    ):
        super().__init__()
        ...
        self.register_buffer('freqs2d', torch.from_numpy(coords[:, 0:2] / extent / 2))
    ...
    def translate_ht(self, img, t, mask=None):
        ...
        coords = self.freqs2d.to(t.device) if mask is None else self.freqs2d[mask] # hack, remove .to
