...
class ImageDataset(data.Dataset):
    ...
    def estimate_normalization(self, n=1000):
        if len(self.src.shape) == 4:
            imgs = self.src.images(range(self.N))[:, 0]
        else:
    ...
    def __len__(self):
        length = self.N * self.multi if self.multi else self.N
        return length
    ...
    def __getitem__(self, index):
        ...
        if self.multi and self.shot:
            structure_idx, view_idx = index // self.multi, index % self.multi
            all_imgs = self.src.images(structure_idx)
            ridxs = np.random.randint(self.multi, size=(self.shot - 1))
            ridxs = np.insert(ridxs, 0, view_idx)
            # ridxs = np.concatenate([[view_idx], ridxs])
            imgs = all_imgs[0][ridxs]
            index = ridxs + (self.multi * structure_idx)
        else:
        ...
        if self.transform:
            if self.contrastive:
                imgs = torch.cat([imgs, self.transform(imgs)], dim=0)
                if isinstance(index, int):
                    index = torch.tensor([index, index])
            else:
...
class TiltSeriesData(ImageDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.tilt_numbers = torch.tensor(self.tilt_numbers)
        ...
        if angle_per_tilt is not None:
            ...
            self.tilt_angles = torch.tensor(self.tilt_angles)
    ...
    def __getitem__(self, index):
        ...
        images = self._process(self.src.images(tilt_indices))
