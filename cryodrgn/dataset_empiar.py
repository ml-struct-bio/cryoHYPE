...
class ImageDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.pub_labels is not None:
            label = self.pub_labels[index]
            index = np.append(np.random.choice(np.where(self.pub_labels == label)[0], self.shot-1), index)
