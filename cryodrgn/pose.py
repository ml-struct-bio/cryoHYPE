...
class PoseTracker(L.LightningModule):
    def __init__(
        self,
        rots_np: np.ndarray,
        ...
        rots = torch.tensor(rots_np.astype(np.float32))
        trans = (
           torch.tensor(trans_np.astype(np.float32))
    ...
    def save(self, out_pkl: str) -> None:
        if self.emb_type == "quat":
            r = lie_tools.quaternions_to_SO3(self.rots_emb.weight.data).numpy()
        elif self.emb_type == "s2s2":
            r = lie_tools.s2s2_to_SO3(self.rots_emb.weight.data).numpy()
        else:
            r = self.rots.numpy()
        ...
        if self.use_trans:
            if self.emb_type is None:
                ...
                t = self.trans_emb.weight.data.numpy()
